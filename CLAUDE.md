...
Reference material — including the procedural actors under `Plugins/Agility/Source/Agility/Procedural/`, `Plugins/Agility/Source/Agility/FlappyBird/`, `Plugins/Agility/Source/Agility/MeshLab/`, and `Plugins/Agility/Shaders/MeshLab.ush` — should **not** be modified when working on a tutorial follower's behalf. Write new code and assets under the host project's `MyContent/` / `MyScripts/` instead. If a session asks you to edit plugin source while following a tutorial, double-check intent — they may be confused about which side they should be working on.
