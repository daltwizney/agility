...
- **Python research workspace** under [`python/`](./python/) — sibling to the plugin (not part of it), home of computer-vision experiments and the web server UE5 talks to as a client. Most CV / deep-learning work is faster to prototype in Python — OpenCV, PyTorch, and modern inference stacks are Python-first — so the algorithm work happens here first, then either ports into the plugin's C++ side or stays in Python with the plugin as a client over HTTP / WebSocket. Empty at the moment; conventions and the why-it-exists in [`python/README.md`](./python/README.md).
